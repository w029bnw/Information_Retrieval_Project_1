...
Original file is located at
    https://colab.research.google.com/drive/1RI5AR9SibOv9iDvDYE9oXG4j3gNEuQV8
"""
...
class QueryProcessor:
    ...
    def vectorQuery(self, k):
        ...
        query_term= preprocessing()
        # vectorizing using term frequency 
        tfidf_values=[]
        cosineSim=[]
        k=[]
        for word in query_terms:
...
#         computing the TF_IDF 
          for termFrequency in postings:
            tfidf_score= term_frequency(word)*idf(word)
            tfidf_rank.append(tfidf_score)
          tfidf_values.append(tfidf_rank, docID)
#        computing the cosine similarity 
          for i in range (len(tfidf_values)):
            cosineSimilarity= ((dot(tfidf_values[i],(tifidf_values[i+1])))/((norm(tfidf_values[i]))*norm(tfidf_values[i+1]))
            cosineSim.append(cosineSimilarity)
#         sorting the ranking and returning them
          
          return cosineSimilarity.sort()
